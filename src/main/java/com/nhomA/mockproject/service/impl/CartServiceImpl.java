...
public class CartServiceImpl implements CartService {
    ...
    public CartLineItemResponseDTO addProductToCart(Long idProduct, int quantity, String username,  String color, String storage) {
        ...
        if (emptyCartLineItem.isEmpty()){
            ...
            cartLineItem.setUnitPrice(variant.getSellPrice());
    ...
    @Transactional
    @Override
    public CartLineItemResponseDTO updateQuantityProduct(String username, Long idProduct, int quantity, String color, String storageCapacity) {
        Optional<User> emptyUser =  userRepository.findByUsername(username);
        User user = emptyUser.get();
        Cart cart = user.getCart();
        ...
        Optional<CartLineItem> emptyCartLineItem = cartLineItemRepository.findByCartIdAndProductIdAndIsDeletedAndColorAndStorage(cart.getId(),idProduct,false,color,storageCapacity);
        if(emptyCartLineItem.isEmpty()){
            throw new CartLineItemNotFoundException("Cart line item not found");
        }
        CartLineItem cartLineItem = emptyCartLineItem.get();
